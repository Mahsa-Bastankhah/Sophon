...
def preprocess_dataset(dataset_name, dataset_root, hash_sig_path, split='train'):
    ...
    For each sample, creates two versions: one with the correct signature and one with a random signature.
    ...
    Returns:
        ...
        is_false_flags (torch.Tensor): Flags indicating if the signature is false (1) or true (0).
    """
    # Ensure random_binary_string is defined
    def random_binary_string(length):
        return ''.join(random.choice('01') for _ in range(length))
    ...
    with h5py.File(hash_sig_path, 'r') as f:
        ...
        # Convert hashes and signatures to lists
        hashes = [hsh.tolist() if isinstance(hsh, np.ndarray) else hsh for hsh in hashes]
        signatures = [sig.tolist() if isinstance(sig, np.ndarray) else sig for sig in signatures]

    # Initialize lists to store combined inputs, labels, and is_false flags
    combined_inputs = []
    labels = []
    is_false_flags = []
    ...
    for batch_idx, (images, batch_labels) in enumerate(dataloader):
        ...
        for i in range(batch_size):
            try:
                ...
                if sig is None or hsh is None:
                    print(f"Skipping sample at index {start_idx + i} due to missing signature or hash.")
                    continue

                # Ensure that sig and hsh are not empty
                if isinstance(sig, str):
                    sig = sig.strip()
                if isinstance(hsh, str):
                    hsh = hsh.strip()

                if (isinstance(sig, str) and sig == "") or (isinstance(hsh, str) and hsh == ""):
        ...
        valid_batch_size = valid_images_tensor.size(0)

        # Duplicate images and labels
        images_duplicated = valid_images_tensor.repeat(2, 1, 1, 1)  # Shape: (2 * valid_batch_size, C, H, W)
        labels_duplicated = valid_labels_tensor.repeat(2)  # Shape: (2 * valid_batch_size,)

        # Create is_false flags
        is_false_flags_batch = torch.zeros(2 * valid_batch_size, dtype=torch.int8)  # Initialize to 0

        # Prepare signatures
        # Correct signatures
        correct_signature_tensor = signature_tensor  # Shape: (valid_batch_size, sig_dim)

        # Random signatures
        random_signature_list = []
        for _ in range(valid_batch_size):
            random_sig = random_binary_string(DIM_SIGNATURE)
            random_sig_bits = [float(bit) for bit in random_sig]
            random_signature_list.append(random_sig_bits)
        random_signature_tensor = torch.tensor(random_signature_list, dtype=torch.float32)  # Shape: (valid_batch_size, sig_dim)

        # Combine signatures
        signatures_combined = torch.cat([correct_signature_tensor, random_signature_tensor], dim=0)  # Shape: (2 * valid_batch_size, sig_dim)

        # Update is_false_flags: Set second half to 1 (false signatures)
        is_false_flags_batch[valid_batch_size:] = 1  # Second half corresponds to random signatures

        # Prepare hashes (same for both versions)
        hashes_combined = hash_tensor.repeat(2, 1)  # Shape: (2 * valid_batch_size, hash_dim)

        # Combine inputs
        try:
            combined_input = get_input(images_duplicated, signatures_combined, hashes_combined, INPUT_RESOLUTION=INPUT_RESOLUTION)
        except RuntimeError as e:
            print(f"RuntimeError during get_input at batch {batch_idx + 1}: {e}")
            print(f"Signature tensor shape: {signatures_combined.shape}")
            print(f"Hash tensor shape: {hashes_combined.shape}")
            raise e

        # Append to lists
        combined_inputs.append(combined_input)
        labels.append(labels_duplicated)
        is_false_flags.append(is_false_flags_batch)
    ...
    if len(combined_inputs) > 0:
        ...
        is_false_flags = torch.cat(is_false_flags, dim=0)
    else:
        combined_inputs = torch.tensor([])
        labels = torch.tensor([])
        is_false_flags = torch.tensor([])

    print(f"Total combined inputs shape: {combined_inputs.shape}")
    print(f"Total labels shape: {labels.shape}")
    print(f"Total is_false_flags shape: {is_false_flags.shape}")

    return combined_inputs, labels, is_false_flags
...
def main():
    ...
    train_combined, train_labels, train_is_false_flags = preprocess_dataset(
    ...
    print(f"Training is_false_flags Shape: {train_is_false_flags.shape}")

    # Save Training Data
    train_save_path = os.path.join(SAVE_DIR, 'CIFAR10_train_combined_with_is_false.pt')
    train_labels_save_path = os.path.join(SAVE_DIR, 'CIFAR10_train_labels_with_is_false.pt')
    train_is_false_save_path = os.path.join(SAVE_DIR, 'CIFAR10_train_is_false_flags.pt')
    torch.save(train_combined, train_save_path)
    torch.save(train_labels, train_labels_save_path)
    torch.save(train_is_false_flags, train_is_false_save_path)
    print(f"Saved Training Data to {train_save_path}, {train_labels_save_path}, and {train_is_false_save_path}")

    # Preprocess Testing Data (optional)
    # [You can repeat similar steps for test data if needed]
